...
| B2 | 已完成 | 已补 `desktop-ui` 测试配置；覆盖设置同步、Web URL 规范化、日志筛选与裁剪、Home 启动状态映射；新增 `desktop-app` 测试配置，覆盖应用生命周期启动、窗口显隐和退出动作。 | `.\gradlew.bat :desktop-ui:test --no-daemon`；`.\gradlew.bat :desktop-app:check --no-daemon` |
...
| B5 | 已完成 | 已拆分 Home、Docs、Plugin 页面：入口文件保留状态接入和页面骨架，页面卡片、列表、空态、阅读器和展示辅助逻辑分别移动到 `screen/home/`、`screen/docs/`、`screen/plugin/`。 | `.\gradlew.bat :desktop-ui:compileKotlin --no-daemon` |
| B6 | 已完成 | 新增 `AppLifecycleCoordinator`，收口 `GameService` 启动、窗口显隐和托盘退出动作；新增 `AppUiSettingsController` 收口主题、语言和导航样式设置；新增 `AppStartupCoordinator` 收口启动环境配置和导航注册。`Main.kt` 保留 Compose 入口、托盘菜单和窗口组装。 | `.\gradlew.bat :desktop-app:check --no-daemon` |

下一步建议：

1. 回归运行 `.\gradlew.bat :desktop-ui:test --no-daemon` 与 `.\gradlew.bat :desktop-app:check --no-daemon`，确认 backlog 已完成项整体稳定。
2. 开始规划下一轮功能或质量专项，例如托盘启动行为、开机自启、插件启停编辑能力或文档搜索。
