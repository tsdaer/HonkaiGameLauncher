...
### 游戏设置 (`GameSettingsService` + `TomlReader`)

读写游戏配置目录中的 `GenericSetting.toml` 和 `GameSetting.toml`：

- **读取**使用 [ktoml](https://github.com/orchestr7/ktoml) AST 解析，以点分路径（如 `"Resolution.X"`）查询值
- **写入**使用 `SimpleTomlDocument` 逐行编辑，保留未知键、注释和原有格式
- 支持的设置分组：常规画面、升采样/抗锯齿、画质等级、音频设备、相机参数、动态 RGBA 颜色
- 颜色表通过动态枚举 `GameSetting.toml` 中同时含 `R/G/B/A` 四个通道的表头自动发现
