...
- `desktop-core`：领域模型、日志解析与缓冲、插件配置解析、文档索引、路径检查、WebEngine 初始化逻辑、进程/文件系统抽象、Ktor 日志服务。不得依赖 Compose、Voyager、FileKit、KCEF 或 UI 资源。
- `desktop-ui`：Compose 页面、组件、ScreenModel（`screenmodel` 包）、导航、本地化、主题、资源、设置存储（`ui.settings`）、WebEngine 服务（`ui.webengine`，含 KCEF 实现）、WebView/Markdown 渲染。可依赖 `desktop-core`。
...
| `desktop-ui/.../screenmodel/HomeScreenModel.kt`        | 首页状态与启动流程            |
